...
export function LibraryScreen({ navigation }: Props) {
  const user = useAuthStore((s) => s.user); const client = useQueryClient();
  const query = useQuery({ queryKey: ['books'], queryFn: api.books });
  const importBook = async () => {
    const picked = await DocumentPicker.getDocumentAsync({ type: ['application/pdf', 'application/epub+zip'], copyToCacheDirectory: true });
    if (picked.canceled) return;
    const asset = picked.assets[0]; const form = new FormData();
    form.append('file', { uri: asset.uri, name: asset.name, type: asset.mimeType ?? (asset.name.toLowerCase().endsWith('.pdf') ? 'application/pdf' : 'application/epub+zip') } as unknown as Blob);
    form.append('title', asset.name.replace(/\.(pdf|epub)$/i, ''));
    await api.uploadBook(form); await client.invalidateQueries({ queryKey: ['books'] });
  };
  return <SafeAreaView style={styles.safe} edges={['top']}>
    <View style={styles.header}><View><Text style={styles.eyebrow}>PRIVATE LIBRARY</Text><Text style={styles.heading}>我的书房</Text></View><Pressable onPress={() => void importBook()} style={styles.import}><Text style={styles.importText}>导入书籍</Text></Pressable></View>
